...
func runClient(id int, batch_size *int, done *atomic.Bool, workload *kvs.Workload, resultsCh chan<- Result, servers []string, wg *sync.WaitGroup) {
	defer wg.Done()
...
func main() {
	...
	workers := flag.Int("workers", 16, "Number of goroutines to spin for each client, default is 64")
	...
	resultsCh := make(chan Result, totalWorkers)
	...
	if totalBatches > 0 {
		avgBatchLat := totalDuration / time.Duration(totalBatches)
