...
for node in "${SERVER_NODES[@]}"; do
    ...
    ${SSH} $node "${BASELINE_ROOT}/bin/kvsserver -port 8080 $SERVER_ARGS > \"$LOG_DIR/kvsserver-$node.log\" 2>&1 &"
done
...
for node in "${CLIENT_NODES[@]}"; do
    ...
    # Pass -batch_size and -workers explicitly
    ${SSH} $node "exec -a '$CLIENT_MARKER' ${BASELINE_ROOT}/bin/kvsclient -hosts $SERVER_HOSTS -batch_size $BATCH_SIZE -workers $WORKERS $CLIENT_ARGS > \"$LOG_DIR/kvsclient-$node.log\" 2>&1" &
    CLIENT_PIDS+=($!)
done
...
# Use the root report-tput.py
python3 "${ROOT}/report-tput.py" "$LOG_DIR"
echo